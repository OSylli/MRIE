...
#'
#' @param IV.dat A required data frame containing data on the instrumental variables (IVs) for all
...
#' @param pheno.dat A required data frame containing data on the outcome and the exposure, with a
...
MRIE_EM <- function(IV.dat, pheno.dat, covar.dat = NULL, epsilon = 1e-5){
  ...
  results_var <- matrix(NA, nrow = 3, ncol = 1, dimnames = list(c("variance of error term (exposure)", "variance of error term (outcome)", "correlation"), c("Estimate")))
  ...
  full_para_name <- c(output.rownames, "variance of error term (exposure)", "variance of error term (outcome)", "correlation")
